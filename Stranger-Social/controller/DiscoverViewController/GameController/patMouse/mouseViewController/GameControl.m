...
- (void)uiConfig{
    ...
    _timeBar1 = [[UIView alloc] initWithFrame:CGRectMake(95, 200, 200, 20)];
    _timeBar1.backgroundColor = [UIColor blueColor];
    //[self insertSubview:_timeBar1 aboveSubview:whiteView];
    _timer = [NSTimer scheduledTimerWithTimeInterval:1.0/120.0 target:self selector:@selector(refreshView) userInfo:nil repeats:YES];
    NSTimer *timeTimmer = [NSTimer scheduledTimerWithTimeInterval:15 target:self selector:@selector(stopTimer) userInfo:nil repeats:NO];
    
    UILabel *scoreLabel = [[UILabel alloc]initWithFrame:CGRectMake(80, 15, 100, 50)];
    scoreLabel.text = @"scores:";
    scoreLabel.font = [UIFont fontWithName:@"HelveticaNeue-Light" size:30];
    ...
    NSLayoutConstraint *centerX=[NSLayoutConstraint constraintWithItem:self.scoreView attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeCenterX multiplier:1.0 constant:20];
    
    [self addConstraint:centerX];
    
    
    
    NSLayoutConstraint *centerY=[NSLayoutConstraint constraintWithItem:self.scoreView attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeCenterY multiplier:1.0 constant:-200];
...
{
    //面向对象
    [[AAAGamificationManager sharedManager] addToMainPlayerScore:2];
